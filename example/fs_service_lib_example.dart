...
void main() async {
  ...
  stdout.writeln('--- Initializing FsServiceLib ---');
  await fsServiceLib.init(
    ...
    onCacheHit: (url, stats) =>
        stdout.writeln('CACHE HIT: $url (Ratio: ${stats.hitRatioPercentage})'),
  );
  ...
  await fsServiceLib.db.batchWrite(
    writes: [
      ...
      const DeleteWrite(
        documentPath: '$testCollectionPath/doc-old',
      ),
    ],
  );
  ...
  stdout.writeln('\n--- 10. documentExists ---');
  final exists =
      await fsServiceLib.db.documentExists(documentPath: testDocPath);
