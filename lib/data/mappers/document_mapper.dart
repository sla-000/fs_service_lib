...
class DocumentMapper {
  DocumentMapper({required this.valueUtils, required this.pathUtils});
  ...
  String _metaPrefix = kDefaultMetaPrefix;

  void init({String? metaPrefix}) {
  ...
  (String? id, Document document) _jsonToDocument(JsonObject inJson) {
    ...
    return (id, Document(fields: fields));
  }
  ...
  List<JsonObject> _jsonCollectionToDocuments(JsonObject json) {
    ...
    if (documents is! List<dynamic>?) {
      throw FormatException(
        'Wrong `$metaDocuments` value=`$documents` '
        'in json=`$json`',
      );
    }
