...
class FirestoreRepoImpl implements FirestoreRepo {
  ...
  Future<void> init({
    ...
    firestorePathUtils.init(projectId: projectId, databaseId: databaseId);
  }
  ...
  Future<JsonObject> getCollection({
    ...
    final collectionDocuments = await _getCollectionDocuments(
      documentPath: firestorePathUtils.absolutePathFromRelative(
        collectionParent,
      ),
  ...
  Future<List<JsonObject>> queryCollection({
    ...
    final collectionParent = pathUtils.parent(collectionPath);
    final collectionName = pathUtils.name(collectionPath);
    final parent = firestorePathUtils.absolutePathFromRelative(
      collectionParent,
    );
    ...
    final responseList = await firestore.runQuery(request, parent);
  ...
  Future<List<JsonObject>> getDocumentsByIds({
    ...
    final request = BatchGetDocumentsRequest(documents: documentPaths);

    final responseList = await firestore.batchGet(request, parent);
  ...
  Future<bool> documentExists({required String documentPath}) async {
    final docPath = firestorePathUtils.absolutePathFromRelative(documentPath);
    try {
      await firestore.get(docPath, mask_fieldPaths: ['__name__']);
      return true;
    } on DetailedApiRequestError catch (error) {
      if (error.status == 404) {
        return false;
  ...
  Future<JsonObject> _getDocumentJson({
    ...
    final collectionNames = await _getDocumentCollectionNames(
      absolutePath: documentName,
    );
  ...
  Future<List<JsonObject>> _iterateAllCollectionsOfDocument(
    ...
    for (final collectionName in collectionNames) {
      ...
      final allDocumentsInCollection = await _iterateAllDocumentsOfCollection(
        collectionDocuments,
      );
  ...
  Future<List<String>> _getDocumentCollectionNames({
    ...
    final response = await firestore.listCollectionIds(
  ...
  Future<void> _onDocumentParsed(
    ...
    final absoluteParent = firestorePathUtils.absolutePathFromRelative(
      documentParent,
    );
  ...
  FutureOr<void> updateDocument({
    ...
    final absolutePath = firestorePathUtils.absolutePathFromRelative(
      documentPath,
    );
    ...
    final fields = Map.fromEntries(
      json.entries.map(
        (e) =>
            MapEntry(e.key, documentMapper.valueUtils.fromJsonObject(e.value)),
      ),
    );
  ...
  @override
  Future<void> batchWrite({required List<FirestoreWrite> writes}) async {
    if (writes.isEmpty) {
      return;
    }
        ...
        writes.map((write) {
          switch (write) {
            case UpdateWrite(:final documentPath, :final json):
              final absPath = firestorePathUtils.absolutePathFromRelative(
                documentPath,
              );
              final fields = Map.fromEntries(
                json.entries.map(
              ...
              return Write(
                update: Document(name: absPath, fields: fields),
