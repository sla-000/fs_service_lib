...
void main() {
  group('CachingHttpClient tests', () {
    ...
    test('caches GET requests', () async {
      ...
      final cachingClient = CachingHttpClientMemory(inner: mockClient);
    ...
    test(
      'tracks stats and triggers onCacheHit callback on cache hit',
      () async {
        String? cacheHitUrl;
    ...
    test(
      'evicts LRU entries when maxSizeBytes memory limit is reached',
      () async {
        final mockClient = MockClient(
          (request) async => http.Response('data:${request.url.path}', 200),
        );

        // Response 'data:/1' is 7 bytes. maxSizeBytes=14 allows max 2 items.
        final cachingClient = CachingHttpClientMemory(
          inner: mockClient,
          maxSizeBytes: 14,
        );
    ...
    test('does not cache responses exceeding maxEntrySizeBytes', () async {
      ...
      expect(cachingClient.stats.stores, equals(0));
    });

    test(
      'evicts oldest entries when total maxSizeBytes memory limit is reached',
      () async {
        final mockClient = MockClient(
        ...
        await cachingClient.get(Uri.parse('https://example.com/1'));
        expect(cachingClient.cacheCount, equals(1));
        expect(cachingClient.currentSizeBytes, equals(30));

        // Storing second 30-byte response should evict /1 because 30+30 > 50
        await cachingClient.get(Uri.parse('https://example.com/2'));
        expect(cachingClient.cacheCount, equals(1));
        expect(cachingClient.currentSizeBytes, equals(30));
      },
