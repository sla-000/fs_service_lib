...
void main() {
  ...
  setUpAll(() async {
    ...
    when(() => _mockFirestorePathUtils.rootPath)
        .thenReturn('projects/p/databases/d/documents');
    when(() => _mockFirestorePathUtils.absolutePathFromRelative(any()))
  ...
  group(
    ...
    () {
      group('queryCollection tests - ', () {
        test('executes runQuery with filters and returns mapped results',
            () async {
          when(() => _mockDocumentMapper.valueUtils).thenReturn(ValueMapper());
          when(
            () => _mockProjectsDatabasesDocumentsResource.runQuery(
              any(),
              any(),
            ),
          ).thenAnswer(
            (_) async => [
              RunQueryResponseElement(document: _document1),
            ],
          );
          when(() => _mockDocumentMapper.documentToJson(_document1))
              .thenReturn({'key': 'val1'});
          ...
          verify(() => _mockProjectsDatabasesDocumentsResource.runQuery(
                any(),
                any(),
              )).called(1);
        });
      ...
      group('getDocumentsByIds tests - ', () {
        ...
        test('executes batchGet and maps found documents', () async {
          when(
            () => _mockProjectsDatabasesDocumentsResource.batchGet(
              any(),
              any(),
            ),
          ).thenAnswer(
            (_) async => [
              BatchGetDocumentsResponseElement(found: _document1),
            ],
          );
          when(() => _mockDocumentMapper.documentToJson(_document1))
              .thenReturn({'key': 'val1'});
      ...
      group('documentExists tests - ', () {
        test('returns true when document is found', () async {
          ...
          final exists =
              await easyFirestore.documentExists(documentPath: 'users/id1');
      ...
      group('batchWrite tests - ', () {
        test('returns immediately for empty writes', () async {
          ...
          verifyNever(() => _mockProjectsDatabasesDocumentsResource.commit(
                any(),
                any(),
              ));
        });
        ...
        test('executes commit with UpdateWrite and DeleteWrite', () async {
          when(() => _mockDocumentMapper.valueUtils).thenReturn(ValueMapper());
          when(
            () => _mockProjectsDatabasesDocumentsResource.commit(
              any(),
              any(),
            ),
          ...
          verify(() => _mockProjectsDatabasesDocumentsResource.commit(
                any(),
                any(),
              )).called(1);
        });
      ...
      group('countCollection tests - ', () {
        test('executes runAggregationQuery and returns total count', () async {
            ...
            (_) async => [
              RunAggregationQueryResponseElement(
                result: AggregationResult(
                  aggregateFields: {
                    'total_count': Value(integerValue: '42'),
                  },
                ),
              ),
            ],
          ...
          final count =
              await easyFirestore.countCollection(collectionPath: 'users');
